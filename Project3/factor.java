...
class factor {
	...
	public int execute(executor ext) {
		...
		case 1:
			...
			int count = ext.getStack().size() - 1;
			HashMap<String, pairedVal> temp = null;

			while (count >= 0) {

				temp = ext.getStack().get(count);
			...
			if (inStack) {
				...
				type = ext.getStack().get(count).get(this.id).getType();
				int val = ext.getStack().get(count).get(this.id).getInt(); // assign value based on the id's type
