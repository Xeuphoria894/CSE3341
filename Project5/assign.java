...
class assign {
	...
	public void execute(executor ext) {
		...
		case 1:
			...
			Core type = null;
			int pointer = ext.getTopStackOfMapForFunc().size() - 1;
			HashMap<String, pairedVal> temp = null;

			while (pointer >= 0) {
				temp = ext.getTopStackOfMapForFunc().get(pointer);
			...
			if (inStack) {
				...
				type = ext.getTopStackOfMapForFunc().get(pointer).get(this.id1).getType();
				int val = ext.getTopStackOfMapForFunc().get(pointer).get(this.id1).getInt();
		...
		case 2:
			...
			Core type1 = null;
			int pointer1 = ext.getTopStackOfMapForFunc().size() - 1;
			HashMap<String, pairedVal> temp1 = null;

			while (pointer1 >= 0) {
				temp1 = ext.getTopStackOfMapForFunc().get(pointer1);
			...
			if (inStack1) {
				...
				type1 = ext.getTopStackOfMapForFunc().get(pointer1).get(this.id1).getType();
				int val = ext.getTopStackOfMapForFunc().get(pointer1).get(this.id1).getInt();
		...
		case 3:
			...
			final int originalCount = ext.numOfReference(); // number of reference before execution
			int pointer2 = ext.getTopStackOfMapForFunc().size() - 1;
			Core type2 = null;
			HashMap<String, pairedVal> temp2 = null;

			while (pointer2 >= 0) {
				temp2 = ext.getTopStackOfMapForFunc().get(pointer2);
			...
			if (inStack2) {
				...
				type2 = ext.getTopStackOfMapForFunc().get(pointer2).get(this.id1).getType();
				Integer oldPointer = ext.getTopStackOfMapForFunc().get(pointer2).get(this.id1).getInt();
				...
				ext.updateNumOfReferenceByOne(position); // add one to reference

				/* if id1 is not null, then it lost its original reference */
				if (oldPointer != null) {

					ext.removeNumOfReferenceByOne(oldPointer);
				...
				Integer oldPointer = ext.getStatic().get(this.id1).getInt();
				if (type2 != Core.REF) {
					System.out.println("ERROR: CANNOT ASSIGN NEW CLASS TO A NON-REF VARIABLE");
					System.exit(1);
				}
			...
			if (ext.numOfReference() != originalCount) {
		...
		case 4:
			...
			final int oldCount = ext.numOfReference();
			int pointerId1 = ext.getTopStackOfMapForFunc().size() - 1;
			int pointerId2 = ext.getTopStackOfMapForFunc().size() - 1;
			Integer valId2 = null;
			Core typeId1 = null;
			Core typeId2 = null;
			...
			while (pointerId1 >= 0) {
				tempId1 = ext.getTopStackOfMapForFunc().get(pointerId1);
			...
			while (pointerId2 >= 0) {
				tempId2 = ext.getTopStackOfMapForFunc().get(pointerId2);
			...
			if (inStackId2) { // what if id2 is null??

				typeId2 = ext.getTopStackOfMapForFunc().get(pointerId2).get(this.id2).getType();
				if (typeId2 != Core.REF) {
					System.out.println("ERROR: CANNOT ASSIGN NEW CLASS TO A NON-REF VARIABLE");
					System.exit(1);
				}

				valId2 = ext.getTopStackOfMapForFunc().get(pointerId2).get(this.id2).getInt();

				if (valId2 != null) {
					ext.updateNumOfReferenceByOne(valId2); // add one to reference
				}
			} else {
			...
			if (inStackId1) {
				...
				typeId1 = ext.getTopStackOfMapForFunc().get(pointerId1).get(this.id1).getType();
				Integer oldRef = ext.getTopStackOfMapForFunc().get(pointerId1).get(this.id1).getInt();
				if (typeId1 != Core.REF) {
					System.out.println("ERROR: CANNOT ASSIGN NEW CLASS TO A NON-REF VARIABLE");
					System.exit(1);
				}
				...
				if (oldRef != null) {
					...
					ext.removeNumOfReferenceByOne(oldRef);
				...
				Integer oldRef = ext.getStatic().get(this.id1).getInt();
				if (typeId1 != Core.REF) {
					System.out.println("ERROR: CANNOT ASSIGN NEW CLASS TO A NON-REF VARIABLE");
					System.exit(1);
				}
			...
			if (ext.numOfReference() != oldCount) {
				System.out.println("gc:" + ext.numOfReference());
			}
