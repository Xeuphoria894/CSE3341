...
class If {
	...
	public void semanticCheck() {
		...
		Parser.scope.peek().push(new HashMap<String, Core>()); // push new scope

		this.s_seq1.semanticCheck();

		Parser.scope.peek().pop(); // return back to the origin scope

		if (this.s_seq2 != null) {
			Parser.scope.peek().push(new HashMap<String, Core>());
			this.s_seq2.semanticCheck();
			Parser.scope.peek().pop();
		}
	}
	...
	public void execute(executor ext) {
		...
		try {
			...
			HashMap<String, pairedVal> popOut = ext.popCurrentStackLayer();

			for (Map.Entry<String, pairedVal> entry : popOut.entrySet()) {
