...
public class executor {
	...
	public void initializeHeap(int scope) {
		...
		this.garbageCollector = new int[scope];
	...
	public Stack<HashMap<String, pairedVal>> popCurrentStackOfMapForFunc() {
	...
	public int getNumOfReference(int pos) {

		return this.garbageCollector[pos];
	}

	public void updateNumOfReferenceByOne(int pos) {

		this.garbageCollector[pos]++;
	}

	public void removeNumOfReferenceByOne(int pos) {

		if (this.garbageCollector[pos] > 0) {
			this.garbageCollector[pos]--;
		}
	...
	public int numOfReference() {
		...
		for (int i = 0; i < this.garbageCollector.length; i++) {

			if (this.garbageCollector[i] > 0) {
				result++;
			}
		}
