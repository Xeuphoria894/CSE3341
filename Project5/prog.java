...
class prog {
	...
	public void execute(executor ext) {
		...
		for (int i = 0; i < deadFunc.size(); i++) {
			...
			for (Map.Entry<String, pairedVal> entry : deadFunc.get(i).entrySet()) {
		...
		for (Map.Entry<String, pairedVal> entry : ext.getStatic().entrySet()) {

			record = ext.numOfReference();
