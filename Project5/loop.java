...
class loop {
	...
	public void semanticCheck() {
		...
		Parser.scope.peek().push(new HashMap<String, Core>()); // create new scope for while
	...
	public void execute(executor ext) {
		...
		HashMap<String, pairedVal> deadObj = null;
		int before = 0;
		...
		while (this.cond.execute(ext)) {
			...
			deadObj = ext.popCurrentStackLayer();

			for (Map.Entry<String, pairedVal> entry : deadObj.entrySet()) {
